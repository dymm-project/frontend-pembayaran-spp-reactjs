...
const ListPembayaran = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      `http://localhost:5000/pembayaran?search_query=${keyword}&page=${page}&limit=${limit}`
    );
  ...
  const deleteData = async (Id) => {
    await axios.delete(`http://localhost:5000/pembayaran/${Id}`).then((res) => {
      ...
      <div class="w-8/12 h-full m-10 mx-auto bg-gray-900 shadow-lg rounded-md border-2 border-gray-800">
        <header class="px-5 py-4 border-b-2 border-gray-800 flex justify-between">
          ...
          <div>
            <Link
              to={"/pembayaran/add"}
              className="bg-green-400 hover:bg-green-600 duration-300 w-44 my-5 py-1.5 px-2 rounded-md flex justify-center items-center text-black font-semibold"
            >
              <span className="mr-2"> Add Pembayaran </span>
                  ...
                  <tr key={data.id_pembayaran}>
                    ...
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center">
                        {data.siswa.spp.thn_ajaran}
                      </div>
                    </td>
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center">
                        {data.siswa.kela.kelas} {data.siswa.kela.jurusan}
                      </div>
                    </td>
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                    ...
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center">
                        {data.bulan.bulan} - {data.thn_bayar}
                      </div>
                    </td>
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center">
                    ...
                    <td className="p-2 whitespace-nowrap font-medium border text-red-500 border-gray-200">
                      <div className="flex justify-center">
                        <Link
                          to={`/pembayaran/edit/${data.id_admin}`}
