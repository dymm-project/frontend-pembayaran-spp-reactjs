...
const ListSpp = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      `http://localhost:5000/spp?search_query=${keyword}&page=${page}&limit=${limit}`
    );
  ...
  const deleteData = async (Id) => {
    await axios.delete(`http://localhost:5000/spp/${Id}`).then((res) => {
      toast.success("Data Spp Berhasil Di Hapus", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      <div class="w-8/12 h-full m-10 mx-auto bg-gray-900 shadow-lg rounded-md border-2 border-gray-800">
        <header class="px-5 py-4 border-b-2 border-gray-800 flex justify-between">
          ...
          <div>
            ...
            >
              <span className="mr-2"> Add Spp </span>
        ...
        <form onSubmit={searchData} className="flex justify-end mx-5 my-2 mt-4">
          <input
            ...
            placeholder="Tahun Ajaran, Nominal"
          />
              ...
              <tbody className="text-sm">
                {datas.map((data, index) => (
                  <tr key={data.id_spp}>
                    ...
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center">{data.thn_ajaran}</div>
                    </td>
                    <td className="p-2 whitespace-nowrap font-medium text-white border border-gray-200">
                      <div className="text-center"><FormatRupiah value={data.nominal}/></div>
                    </td>
                    <td className="p-2 whitespace-nowrap font-medium border text-red-500 border-gray-200">
                      <div className="flex justify-center">
                        <Link
                          to={`/spp/edit/${data.id_spp}`}
                        ...
                        <button
                          onClick={() => deleteData(data.id_spp)}
