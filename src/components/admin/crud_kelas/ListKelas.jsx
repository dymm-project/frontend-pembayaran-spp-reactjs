...
const ListKelas = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      `http://localhost:5000/kelas?search_query=${keyword}&page=${page}&limit=${limit}`
    );
  ...
  const deleteData = async (Id) => {
    await axios.delete(`http://localhost:5000/kelas/${Id}`).then((res) => {
      toast.success("Data Kelas Berhasil Di Hapus", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      <div class="w-8/12 h-full m-10 mx-auto bg-gray-900 shadow-lg rounded-md border-2 border-gray-800">
        <header class="px-5 py-4 border-b-2 border-gray-800 flex justify-between">
          ...
          <div>
            ...
            >
              <span className="mr-2"> Add Kelas </span>
                  ...
                  <tr key={data.id_kelas}>
                    ...
                    <td className="p-2 whitespace-nowrap font-medium border text-red-500 border-gray-200">
                      <div className="flex justify-center">
                        ...
                        <button
                          onClick={() => deleteData(data.id_kelas)}
